...
setwd("/Users/peiyaow/Documents/GitHub/continuum/simulation/sparse/fixb")
...
for (j in 2:G){
  b.list[[j]] = matrix(c(rep(0, 20*(j-2)), c(rep(1, 5), rep(0, 20-5)), rep(0, 20*(G-j))))
}
r = seq(0, 0.3, length.out = 6)
s = 1
...
for (a in c(0, 0.5, 1)) {
  ...
  for (k in 1:10){
    # ------------------------- generate data ------------------------------
    X.list = lapply(1:G, function(l) generateX_AR(n, p, r = r[l], s = s))
    eps.list = lapply(1:G, function(l) scale(rnorm(n)))
    Y.list = lapply(1:G, function(l) X.list[[l]]%*%(b.list[[l]]+b) + eps.list[[l]])
    
    X.test.list = lapply(1:G, function(l) generateX_AR(n, p, r = r[l], s = s))
...
save(mse.global,mse.group,mse.int, file = "group_heter_AR.RData")

...
ksi = .1
b.list = list()
b.list[[1]] = matrix(rnorm(5, mean = 0, sd = 1))
for (j in 2:G){
  b.list[[j]] = diag(rbinom(5, size = 1, prob = ksi)*2-1)%*%b.list[[j-1]]
}
b.list = lapply(1:G, function(g) c(b.list[[g]], rep(0, p-5)))
b = matrix(c(rep(1.5, 5), rep(0, p-5)))
